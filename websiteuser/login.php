...
<?php
        if(isset($_POST['username'])){
            ...
            $query = mysqli_query($koneksi, "SELECT*FROM user_ormawa where username='$username' and password='$password'");

            if(mysqli_num_rows($query) > 0){
                $data = mysqli_fetch_array($query);
                $_SESSION['user_ormawa'] = $data;
                echo '<script>alert("Selamat datang, '.$data['nama'].'"); location.href="dashboard.php";</script>';
            }else{
